...
[^3]: Однако, не взлетела. На тестовых данных, поступавших по `COM`-порту с другой программы, все было идеально. Но в боевых условиях все пошло сильно не так. С чем это связано -- пока не ясно. Было решено этого не делать -- коэффициенты подобрались и так. Есть подозрение, что 99% все-таки дело в программе микроконтроллера, а не в данной программе. И, вероятнее всего, подвели неявные приведелния типов `signed` и `unsigned`.
